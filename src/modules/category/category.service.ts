...
export class CategoryService {
  ...
  async findAll(currentPage: number = 1, limit: number = 10) {
    ...
    const [categories, totalCategories] = await Promise.all([
      this.categoryRepository.findAllWithPaginationAndFilter({}, skip, limit),
      this.categoryRepository.countDocument({}),
    ]);
  ...
  async create(categoryData: CreateCategoryDto) {
    ...
    const existingCategory =
      await this.categoryRepository.findByNameExcludingId(normalizedName);
    if (existingCategory) {
    ...
    // await this.notificationService.newCategoryCreation();
  ...
  async update(id: string, categoryData: UpdateCategoryDto) {
    ...
    const normalizedName = categoryData.name.toLowerCase().trim();
    const existingCategory =
      await this.categoryRepository.findByNameExcludingId(normalizedName, id);
    if (existingCategory) {
      throw new ConflictException('Category name already exists');
    }
  ...
  async updateStatus(id: string, status: string) {
    ...
    await this.categoryRepository.updateStatus(id, isActive);
